...
class Point implements PointInterface {
		...
		public function getEncodedPoint($order) {
            if (!($this instanceof Point) && $this == self::$infinity)
                return "00";
			$xhex = "";
			$yhex = "";
			$len = 0;
            if (extension_loaded('gmp') && USE_EXT=='GMP') {
				$xhex = gmp_strval($this->getX(), 16);
				$yhex = gmp_strval($this->getY(), 16);
				// $len = ceil((floatval(strlen(gmp_strval($this->getOrder(), 2))) / 8.0));
				$ohex = gmp_strval($order, 16);
				$len = strlen($ohex);
				if (($len % 2) != 0) $len++;
            } else if (extension_loaded('bcmath') && USE_EXT=='BCMATH') {
            } else {
			...
			while (strlen($xhex) < $len) {
				$xhex = "0" . $xhex;
			}
			while (strlen($yhex) < $len) {
				$yhex = "0" . $yhex;
			}
			return "04" . $xhex . $yhex;
		}

		public static function decodePoint($curve, $s) {
			if ($s === "00") {
				return Point::infinity;
			} else if ((strpos($s, "04", 0) === 0) && ((strlen($s) %2) == 0)) {
				$l = (strlen($s) - 2) / 2;
				if (extension_loaded('gmp') && USE_EXT=='GMP') {
					$x = gmp_init(substr($s, 2, $l), 16);
					$y = gmp_init(substr($s, 2 + $l, $l), 16);
                ...
                throw new ErrorException("Invalid point encoding.");
			}
		}
